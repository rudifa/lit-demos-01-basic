// https://github.com/open-wc/lit-demos/blob/master/01-basic/02-manage-properties.js
...
class ManageProperties extends LitElement {
  ...
  constructor() {
    ...
    this.message = "hello open-wc lit-element"
    this.count = 0;
  }
  ...
  render() {
    return html`
      <div>
        ...
        <div>
          The message is: <i>${this.message}</i>, count is: ${this.count}
        </div>
        <!-- Call a function and render the return value -->
        <div>
          The reversed message is: <i>${this.reverseMessage(this.message)}</i>
        </div>
      </div>
    `;
  }
